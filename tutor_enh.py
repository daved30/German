...
async def play_voice(text):
    ...
    try:
        ...
        clean_text = text.encode('ascii', 'ignore').decode('ascii').replace('*', '').strip()
...
async def conversation_loop():
    ...
    while True:
        try:
            ...
            en_translation = ollama_call(de_text, "Translate this German to English. Only the translation.")
            print(f"\nYOU (DE): {de_text}")
            print(f"YOU (EN): {en_translation}")

            # 4. Get the AI's German response
            ai_reply_de = ollama_call(de_text, "You are a friendly German tutor. Simple German only. 1-2 sentences.")
            if ai_reply_de:
                # 5. Translate AI response and Speak
                ai_reply_en = ollama_call(ai_reply_de, "Translate this German to English. Only the translation.")
